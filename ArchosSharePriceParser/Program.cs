    ...
    class Program
    {
        ...
        public static void ParseAverage(TextFieldParser parser)
        {
            ...
            while (!parser.EndOfData)
            {
                ...
                averageEntry.OpenPrice += float.Parse(values[1], info);
                averageEntry.ClosePrice += float.Parse(values[2], info);
                averageEntry.HighPrice += float.Parse(values[3], info);
                averageEntry.LowPrice += float.Parse(values[4], info);
                averageEntry.Volume += int.Parse(values[5], info);
                    ...
                    lastDateTime.Year != dateTime.Year)
                {
                    ...
                    Console.WriteLine(string.Format("{0}-{1},{2},{3},{4},{5}", averageEntry.Month, averageEntry.Year,
